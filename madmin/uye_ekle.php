...
if(@$_POST['gonder'] and @$_GET['islem']=="")
{
	...
	$firma_gorevi  	=	$_POST["firma_gorevi"];
	$sektoru  	=	$_POST["sektoru"];
	$sifre  	=	$_POST["sifre"];
	
	$query = $db->query("SELECT * FROM kullanici WHERE eposta = '$e_posta'")->fetch(PDO::FETCH_ASSOC);
...
if(@$_GET['islem']=="duzenle" and @$_POST['gonder'])
{
	...
	if(!filter_var($e_posta,FILTER_VALIDATE_EMAIL))
	{
		...
		if(!empty($sifre_yeni))
		{
					...
					$update = $db->prepare("UPDATE kullanici SET 
						ad_soyad 	= :ad_soyad,
						eposta 	= :eposta,
						telefon = :telefon,
						adres 	= :adres,
...
{
	$duzenlenecek_id = $_GET['id'];
	$yonetici_kaydi = $db->query("SELECT * FROM kullanici WHERE id = '$duzenlenecek_id'")->fetch(PDO::FETCH_ASSOC);
}
          ...
          <div class="box box-primary col-md-12">
            ...
            <form role="form" action="" method="post" enctype="multipart/form-data">
                <div class="form-group col-md-12">
                  <label class="control-label">Üye Ad Soyad</label>
                     <div class="controls">
                ...
                <div class="form-group col-md-12">
                  <label class="control-label">Firma Adı</label>
                     <div class="controls">
                         <input type="text" class="form-control" name="firma_adi" value="<?=@$yonetici_kaydi['firma_adi']?>">
                      <span class="help-inline"></span>
                     </div>
                </div>
                <div class="form-group col-md-12">
                  <label class="control-label">Firma Görevi</label>
                     <div class="controls">
                         <input type="text" class="form-control" name="firma_gorevi" value="<?=@$yonetici_kaydi['firma_gorevi']?>">
                      <span class="help-inline"></span>
                     </div>
                </div>
                <div class="form-group col-md-12">
                  <label class="control-label">Sektörü</label>
                     <div class="controls">
                         <input type="text" class="form-control" name="sektoru" value="<?=@$yonetici_kaydi['sektoru']?>">
                      <span class="help-inline"></span>
                     </div>
                </div>
                 <div class="form-group col-md-12">
                  <label class="control-label">Telefon Numaranız</label>
                    <div class="controls">
                        <input type="text" class="form-control" name="telefon" value="<?=@$yonetici_kaydi['telefon']?>"><br>
                    </div>
                </div>
                <div class="form-group col-md-12">
                  <label class="control-label">Adresiniz</label>
                    <div class="controls">
                        <textarea style="height:100px" class="form-control" name="adres"><?=@$yonetici_kaydi['adres']?></textarea><br>
                    </div>
                </div>             
                <?php
				  if(@$_GET['islem']=="duzenle")
